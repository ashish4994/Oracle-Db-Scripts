/*
  Script:      700_DBAatOPS_MK1050_SYN_RO_VAL.sql
...
spo 700_DBAatOPS_MK1050_SYN_RO_VAL_&filename1..log;
...
BEGIN		      
	...
	V_SYN_NAME_LIST(1).PROC_NAME 	:= 'PKG_MS_REWARDS_SIX_FLAGS_ITEM_P';
	V_SYN_NAME_LIST(1).SCHEMA_NAME 	:= 'REWARD';	
	
	V_SYN_NAME_LIST(2).SYN_NAME 	:= 'PKG_MS_REWARDS_OFFER_TYPE';
	V_SYN_NAME_LIST(2).PROC_NAME 	:= 'PKG_MS_REWARDS_OFFER_TYPE_P';
	V_SYN_NAME_LIST(2).SCHEMA_NAME 	:= 'REWARD';	
	
	V_SYN_NAME_LIST(3).SYN_NAME 	:= 'PKG_MS_REWARDS_SUMMARY';
	V_SYN_NAME_LIST(3).PROC_NAME 	:= 'PKG_MS_REWARDS_SUMMARY_P';
	V_SYN_NAME_LIST(3).SCHEMA_NAME 	:= 'REWARD';
    
	-- Validate that synonyms exist and that are pointing to the right packages.
	FOR I IN V_SYN_NAME_LIST.FIRST .. V_SYN_NAME_LIST.LAST LOOP 
		...
		SELECT TABLE_NAME
		  INTO V_OBJECT_NAME
		  FROM DBA_SYNONYMS
		 WHERE SYNONYM_NAME = V_SYN_NAME_LIST(I).SYN_NAME
		   AND OWNER = 'PUBLIC'
		   AND TABLE_OWNER = V_SYN_NAME_LIST(I).SCHEMA_NAME;
		
		IF V_OBJECT_NAME = V_SYN_NAME_LIST(I).PROC_NAME THEN
