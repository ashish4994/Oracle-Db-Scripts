/*
  Script:      VALIDATE_INSERT_EMAIL_REDEM_INTO_T_LU_NOTIFICATION_TMPL_VAR.sql
  Schema:      NOTIFY
...
BEGIN		      
	...
	FOR I IN V_TMPL_LIST.FIRST .. V_TMPL_LIST.LAST LOOP 
		...
		FOR J IN V_VAR_LIST.FIRST .. V_VAR_LIST.LAST LOOP 
			...
			-- Check if the template var exists. 
			SELECT COUNT(1)
			  INTO V_COUNT
			...
			IF V_COUNT = 1 THEN
				...
				IF V_VAR_LIST(J).REC_VARIABLE_NAME != 'MISCELLANEOUS18' OR V_TMPL_LIST(I).REC_HAS_OPERATING_SEASON = 1 THEN										
					DBMS_OUTPUT.PUT_LINE('Template var '''|| V_VAR_LIST(J).REC_VARIABLE_NAME ||''' for template code '''|| V_TMPL_LIST(I).REC_TMPL_CODE ||''' exists. VALIDATION SUCCEDED');				
				ELSE                                                   
					RAISE_APPLICATION_ERROR(-20002, 'Template var '''|| V_VAR_LIST(J).REC_VARIABLE_NAME ||''' for template code '''|| V_TMPL_LIST(I).REC_TMPL_CODE ||''' does not exist. VALIDATION FAILED.');  			
				END IF; 
