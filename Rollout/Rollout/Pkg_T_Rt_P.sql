CREATE OR REPLACE PACKAGE IFLOW.PKG_T_RT_P AS
...
END PKG_T_RT_P;
/
CREATE OR REPLACE PACKAGE BODY IFLOW.PKG_T_RT_P IS
    ...
    ******************************************************************************/
    ...
    IS
        ...
        v_cardnumber number;
		v_internal_product_name varchar2(100);
    BEGIN
		...
		-- get the Internal Product Group Name  
		BEGIN
			SELECT pg.product_group_desc
			INTO v_internal_product_name
			FROM t_credit_account ca
			JOIN t_card tc
				ON ca.credit_account_id = tc.credit_account_id
			JOIN t_lu_product_group_program pgp
				ON ca.source_code = pgp.program_code
			JOIN t_lu_product_group pg
				ON pgp.product_group_code = pg.product_group_code
			WHERE tc.card_num = cardNumber;
			EXCEPTION
				WHEN NO_DATA_FOUND THEN
					v_internal_product_name := '';
		END;
        ...
        OPEN i_ref FOR
            SELECT (SELECT lu.lookup_desc
				   ...
				   v_internal_product_name AS "internal_product_name",
...
GRANT EXECUTE ON IFLOW.PKG_T_RT_P TO CUSTOMER;

GRANT EXECUTE ON IFLOW.PKG_T_RT_P TO DLSRVC;

GRANT EXECUTE ON IFLOW.PKG_T_RT_P TO WEB_RO;
