/*
  Script:      VALIDATE_DELETE_EMAIL_REDEM_INTO_T_LU_NOTIFICATION_VARIABLE.sql
  Schema:      NOTIFY
...
BEGIN		      
	...
	FOR I IN V_LIST.FIRST .. V_LIST.LAST LOOP 
		...
		IF V_COUNT = 0 THEN		  
			DBMS_OUTPUT.PUT_LINE('Variable '''|| V_LIST(I).REC_NAME ||''' not found. VALIDATION SUCCEDED');
		ELSE                                                   
			RAISE_APPLICATION_ERROR(-20002, 'Variable '''|| V_LIST(I).REC_NAME ||''' still exists. VALIDATION FAILED.');
		END IF;	  
