/*
  Script:      100_REWARDatOPS_MK1050_DDL_RB_VAL.sql
  Author:      Levi Tamiozzo
  Date:        01/26/2021
  Purpose:     Validates drop table 	T_REWARDS_SIX_FLAGS_ITEM.
			   Validates drop sequence 	SEQ_REWARDS_SIX_FLAGS_ITEM.
			   
			   Validates drop table 	T_REWARDS_SIX_FLAGS_ITEM_CODE.
			   Validates drop sequence 	SEQ_REWARDS_SIX_FLAGS_ITEM_CODE.
			   
			   Validates drop new columns 		T_REWARDS_REDEMPTION_SIX_FLAGS.
			   Validates drop new columns 	SEQ_REWARDS_REDEMPTION_SIX_FLAGS.
*/
...
spool  100_REWARDatOPS_MK1050_DDL_RB_VAL_&filename1..log
...
DECLARE
    ...
    STR_VALIDATION_TYPE VARCHAR2(100) := 'ROLLBACK VALIDATION for DDL Reward changes';
...
BEGIN
	...
	FOR I IN V_VAL_OBJ_LIST.FIRST .. V_VAL_OBJ_LIST.LAST LOOP 
		...
		IF (V_COUNT = 0) THEN
		  STR_AUDIT := STR_AUDIT || V_VAL_OBJ_LIST(I).OBJ_TYPE || ' ' || V_VAL_OBJ_LIST(I).OBJ_NAME || ' has been dropped ' || chr(13);   
		ELSE
		  STR_ERR := STR_ERR || 'ROLLBACK VALIDATION FAILED CHECKING FOR ' || V_VAL_OBJ_LIST(I).OBJ_TYPE || ' ' || V_VAL_OBJ_LIST(I).OBJ_NAME || chr(13); 
		END IF;
		
	END LOOP;

	v_table_name  := 'T_REWARD_TRANSACTION_HISTORY';
	...
	IF (v_count = 0) THEN
		str_audit := str_audit || 'new columns have been dropped to table ' || v_table_name || chr(13);		
	ELSE
	...
	IF (v_count = 0) THEN
		str_audit := str_audit || 'new columns have been dropped from table ' || v_table_name || chr(13);		
	ELSE
		str_err := str_err || 'VALIDATION FAILED CHECKING FOR NEW COLUMNS IN TABLE ' || v_table_name || chr(13);	
	END IF;	
