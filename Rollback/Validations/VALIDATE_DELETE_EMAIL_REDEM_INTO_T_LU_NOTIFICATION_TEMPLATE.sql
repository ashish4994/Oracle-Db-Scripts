/*
  Script:      VALIDATE_DELETE_EMAIL_REDEM_INTO_T_LU_NOTIFICATION_TEMPLATE.sql
  Schema:      NOTIFY
  Author:      Levi Tamiozzo
  Project:     MK-1050
  Date:        2021-02-04
  Purpose:     Validate delete templates for Six Flags REDEM Emails.
*/
...
BEGIN		      
	...
	FOR I IN V_LIST.FIRST .. V_LIST.LAST LOOP 
		...
		IF V_COUNT = 0 THEN		  
			DBMS_OUTPUT.PUT_LINE('Template '''|| V_LIST(I).REC_TMPL_CODE ||''' not found. VALIDATION SUCCEDED');
		ELSE                                                   
			RAISE_APPLICATION_ERROR(-20002, 'Template '''|| V_LIST(I).REC_TMPL_CODE ||''' still exists. VALIDATION FAILED.');
		END IF;	  
