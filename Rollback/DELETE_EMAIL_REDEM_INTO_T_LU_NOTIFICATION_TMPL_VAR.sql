/*
  Script:      DELETE_EMAIL_REDEM_INTO_T_LU_NOTIFICATION_TMPL_VAR.sql
  Schema:      NOTIFY
...
BEGIN		      
	...
	FOR I IN V_TMPL_LIST.FIRST .. V_TMPL_LIST.LAST LOOP 
		...
		-- Delete all if there is any.
		IF V_COUNT > 0 THEN
		
			DELETE FROM T_LU_NOTIFICATION_TMPL_VAR T
				  WHERE T.NOTIFICATION_TMPL_CODE = V_TMPL_LIST(I).REC_TMPL_CODE;
				
			DBMS_OUTPUT.PUT_LINE('Template vars for template code '''|| V_TMPL_LIST(I).REC_TMPL_CODE ||''' deleted succesfuly.');
		ELSE                                                   
			DBMS_OUTPUT.PUT_LINE('Non template vars for template code '''|| V_TMPL_LIST(I).REC_TMPL_CODE ||''' were found, no data was changed.');
		END IF;
